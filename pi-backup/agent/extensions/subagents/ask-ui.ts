...
// ── Interactive ask runner (vendored execute-body from pi-ask-user) ──────────
// Branch A of the unified `ask_user` tool: runs the genuine interactive UI in
// a human-attended top-level process. The overlay-hide/reopen raw-input
// listener from upstream (ctx.ui.onTerminalInput) is omitted — a nice-to-have
// that only matters while the overlay is hidden.

export interface AskRunResult {
	content: Array<{ type: "text"; text: string }>;
	isError?: boolean;
	details: AskToolDetails | { error: string };
}

export async function runInteractiveAsk(
	pi: ExtensionAPI,
	...
	if (options.length === 0) {
		...
		const answer = await ctx.ui.input(prompt, "Type your answer...", timeout ? { timeout } : undefined) as
			| string
			| undefined;
	...
	try {
		...
		const customFn = ctx.ui.custom as (<T>(f: unknown, o?: unknown) => Promise<T | undefined>);
		const customResult = await customFn<AskUIResult | null>(
			customFactory,
			buildCustomUIOptions(effectiveDisplayMode),
		);
